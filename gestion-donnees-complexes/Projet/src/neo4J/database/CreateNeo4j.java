...
import neo4J.csv.ReadCSV_Departements;
import neo4J.csv.ReadCSV_Regions;
...
public class CreateNeo4j {
	...
	private static final String DB_PATH = "neo4j-database/regionsDepartements";
			...
			for(String[] reg : regions){
				...
				for(String [] dep: departements){
					if(dep[0].equals(reg[0])){
						...
						relationship = departementNode.createRelationshipTo(regionNode, RelTypes.BOLEONGSTO );
	...
	void removeData()
	{
		...
		try 
		{
			...
			//regionNode.getSingleRelationship( RelTypes.BOLEONGSTO, Direction.OUTGOING ).delete();
			//regionNode.delete();
